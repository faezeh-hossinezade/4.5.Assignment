...
img = cv2.imread("OpenVtuber/Input/mrbean input.jpg") 
color = (0, 0, 255)
color2 = (125, 255, 125)
...
for pred in fa.get_landmarks(img , boxes):
    for index , p in enumerate(np.round(pred).astype(np.int8)):
        ...
...
cv2.imwrite("OpenVtuber/Output/mrbean output.jpg",img)
cv2.waitKey()